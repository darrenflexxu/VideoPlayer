...
struct DllInit {
  DllInit() { VideoPlayer::initPlayer(); }
};
...
bool VideoPlayer::startPlay(const char* filePath) {
  ...
  if (filePath)
    file_path_ = filePath;
  // 启动新的线程实现读取视频文件
  std::thread([&](VideoPlayer* pointer) { pointer->readVideoFile(); }, this)
      .detach();
  return true;
}
...
bool VideoPlayer::play() {
  ...
  video_start_time_ +=
      pauseTime;  // 将暂停的时间加到开始播放的时间上，保证同步不受暂停的影响
  player_state_ = VideoPlayer_Playing;
  doPlayerStateChanged(VideoPlayer_Playing, video_stream_ != nullptr,
                       audio_stream_ != nullptr);
  return true;
}
...
bool VideoPlayer::pause() {
  ...
  doPlayerStateChanged(VideoPlayer_Pause, video_stream_ != nullptr,
                       audio_stream_ != nullptr);
  return true;
}
...
int VideoPlayer::openSDL() {
  ...
  for (int i = 0; i < num; i++) {
    audio_id_ = SDL_OpenAudioDevice(SDL_GetAudioDeviceName(i, 0), false,
                                    &wanted_spec, &spec, 0);
    if (audio_id_ > 0) {
      break;
    }
...
void VideoPlayer::readVideoFile() {
  ...
  if (videoStream >= 0) {
    ...
    /// 创建一个线程专门用来解码视频
    std::thread([&](VideoPlayer* pointer) { pointer->decodeVideoThread(); },
                this)
        .detach();
  }
  ...
  if (audioStream >= 0) {
    /// 查找音频解码器
    // 1. 取出流和参数
    AVStream* audio_stream = format_ctx_->streams[audioStream];
    AVCodecParameters* audio_codecpar = audio_stream->codecpar;

    // 2. 查找解码器
    audio_codec_ = avcodec_find_decoder(audio_codecpar->codec_id);
    ...
    if (audio_codec_ == NULL) {
      ...
      // 输出的声道布局
      av_channel_layout_default(&out_ch_layout, audio_tgt_channels_);
      out_ch_layout.nb_channels &= ~AV_CH_LAYOUT_STEREO_DOWNMIX;
      /// 这里音频播放使用了固定的参数
      ...
      // 输入的声道布局
      in_ch_layout = audio_codec_ctx_->ch_layout;
      // 输出的采样率
      out_sample_rate_ = 44100;
      // 输出的声道布局
      audio_tgt_channels_ =
          2;  /// av_get_channel_layout_nb_channels(out_ch_layout);
      av_channel_layout_default(&out_ch_layout, audio_tgt_channels_);
      out_ch_layout.nb_channels &= ~AV_CH_LAYOUT_STEREO_DOWNMIX;
      /// wav/wmv
      /// 文件获取到的audio_codec_ctx_->channel_layout为0会导致后面的初始化失败，因此这里需要加个判断。
  ...
  while (1) {
    ...
    if (seek_req_) {
      ...
      if (stream_index >= 0) {
        seek_target =
            av_rescale_q(seek_target, aVRational,
                         format_ctx_->streams[stream_index]->time_base);
      }

      if (av_seek_frame(format_ctx_, stream_index, seek_target,
    ...
    // 防止一下子就把视频读完了，导致的空间分配不足 这个值可以稍微写大一些
    if (audio_pack_list_.size() > MAX_AUDIO_SIZE ||
        video_pack_list_.size() > MAX_VIDEO_SIZE) {
      mSleep(10);
      continue;
    }
  ...
  SDL_Quit();
  doPlayerStateChanged(VideoPlayer_Stop, video_stream_ != nullptr,
                       audio_stream_ != nullptr);
...
/// 播放器状态改变的时候回调此函数
void VideoPlayer::doPlayerStateChanged(const VideoPlayerState& state,
                                       const bool& hasVideo,
                                       const bool& hasAudio) {
  fprintf(stderr, "%s \n", __FUNCTION__);
...
void VideoPlayer::doDisplayVideo(const uint8_t* yuv420Buffer,
                                 const int& width,
                                 const int& height) {
  //    fprintf(stderr, "%s \n", __FUNCTION__);
  if (video_player_callback_ != nullptr) {
